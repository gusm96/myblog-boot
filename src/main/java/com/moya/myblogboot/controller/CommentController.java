...
public class CommentController {
    ...
    public ResponseEntity<List> requestCommentList(@PathVariable("boardId") Long boardId) {
        return ResponseEntity.ok().body(commentService.retrieveComments(boardId));
    }

    // 자식 댓글 리스트
    @GetMapping("/api/v1/comments/child/{parentId}")
    public ResponseEntity<List> requestChildCommentList(@PathVariable("parentId") Long parentId) {
        return ResponseEntity.ok().body(commentService.retrieveChildComments(parentId));
    }
