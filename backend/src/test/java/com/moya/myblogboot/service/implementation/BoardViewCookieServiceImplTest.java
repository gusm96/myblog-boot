...
class BoardViewCookieServiceImplTest {

    // 쿠키 형식: {date}:{boardId1},{boardId2},...|{HMAC_SIGNATURE}

    private static final String TEST_SECRET = "test-board-view-hmac-secret-key-32b!";
    private static final ZoneId KST = ZoneId.of("Asia/Seoul");

    private BoardViewCookieServiceImpl service;

    @BeforeEach
    void setUp() {
        service = new BoardViewCookieServiceImpl(TEST_SECRET);
    }
    ...
    @Test
    @DisplayName("addViewed — 최초 생성 (cookieValue == null): {date}:{boardId}|{sig}")
    void addViewed_최초생성() {
        Long boardId = 42L;
        String result = service.addViewed(null, boardId);
        ...
        assertThat(payload).isEqualTo(today + ":" + boardId);
    ...
    @Test
    @DisplayName("addViewed — 기존 쿠키에 새 boardId 추가: 기존 ids 뒤에 _{boardId} 추가")
    ...
    void addViewed_날짜_섹션_하나() {
        ...
        // "yyyy-MM-dd:1_2_3" 형태여야 함 — ':' 는 날짜 구분자 1개여야 함
    ...
    void isValid_다른_시크릿() {
        BoardViewCookieServiceImpl otherService =
                new BoardViewCookieServiceImpl("other-secret-key-32bytes-minimum!!");
    ...
    @Test
    @DisplayName("isViewed — 단일 boardId: 포함된 경우 → true")
    void isViewed_단일_포함() {
        String cookie = service.addViewed(null, 42L);
    ...
    @Test
    @DisplayName("isViewed — 단일 boardId: 없는 경우 → false")
    ...
    @Test
    @DisplayName("isViewed — 여러 boardId 중 포함 여부")
    void isViewed_복수() {
        String cookie = service.addViewed(null, 1L);
        cookie = service.addViewed(cookie, 2L);
        cookie = service.addViewed(cookie, 3L);
